from PolynomialModel.ModelStats import test
from PolynomialModel import transform_dataset
import DataUtils
import numpy as np
...
def grad_f(w: np.array, x: np.array, y: np.array, lamda: float, reg_type: str) -> np.array:
    ...
    temp = w.copy()
    temp[0] = 0
    if reg_type=="L1GD": w_grad = -1*(res.transpose().dot(x)).flatten() + lamda*(abs(temp)/w)
    else: w_grad = -1*(res.transpose().dot(x)).flatten() + lamda*temp
    return w_grad
...
def grad_desc(x_train, y_train, error, alpha, epsilion, lamda, reg_type):
    ...
    while True:
        ...
        print(f"MSE: {new_error}, \tRMSE: {new_error**0.5}")
    return w
